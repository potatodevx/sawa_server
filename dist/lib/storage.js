...
exports.isImageStorageConfigured = isImageStorageConfigured;
...
exports.createPresignedDownload = createPresignedDownload;
exports.uploadBuffer = uploadBuffer;
exports.materializeImage = materializeImage;
exports.materializeImageLoose = materializeImageLoose;
exports.materializeImages = materializeImages;
exports.deleteObjectByUrlOrKey = deleteObjectByUrlOrKey;
 ...
 */
function isImageStorageConfigured() {
    return Boolean(isStorageConfigured() && env_1.env.APP_URL);
}
function getClient() {
    if (_client)
        return _client;
 ...
 */
function bucketForKey(_key) {
    return env_1.env.S3_IMAGE_BUCKET || env_1.env.S3_BUCKET;
}
function bucketForFolder(folder) {
    return folder === 'image' ? env_1.env.S3_IMAGE_BUCKET || env_1.env.S3_BUCKET : env_1.env.S3_BUCKET;
}
/**
...
function publicUrlForKey(key) {
    if (isImageKey(key) && env_1.env.APP_URL) {
        return `${env_1.env.APP_URL.replace(/\/$/, '')}/img/${key}`;
    }
    if (!isImageKey(key) && env_1.env.S3_PUBLIC_BASE_URL) {
        return `${env_1.env.S3_PUBLIC_BASE_URL.replace(/\/$/, '')}/${key}`;
    }
    const endpoint = (env_1.env.S3_ENDPOINT || '').replace(/\/$/, '');
...
async function getObjectStream(key) {
    if (!isStorageConfigured() || !key)
        return null;
    try {
        const out = await getClient().send(new client_s3_1.GetObjectCommand({ Bucket: bucketForKey(key), Key: key }));
        if (!out.Body)
            return null;
        return {
            body: out.Body,
        ...
        logger_1.logger.warn(`[storage] getObjectStream(${key}) failed:`, err);
        return null;
    }
}
const EXT_BY_FOLDER = {
...
async function createPresignedUpload(opts) {
    ...
    const key = `${opts.folder}/${safeCouple}/${Date.now()}-${(0, crypto_1.randomUUID)()}.${ext}`;
    const command = new client_s3_1.PutObjectCommand({
        Bucket: bucketForFolder(opts.folder),
        Key: key,
        ContentType: opts.contentType,
    });
    const uploadUrl = await (0, s3_request_presigner_1.getSignedUrl)(getClient(), command, {
 ...
 */
async function createPresignedDownload(key, expiresInSeconds = 6 * 60 * 60) {
    if (!isStorageConfigured()) {
        throw new Error('Object storage is not configured');
    }
 ...
 */
async function materializeImage(value, coupleId) {
    if (!value || typeof value !== 'string')
        return value;
    if (!value.startsWith('data:'))
        return value; // already a URL (or non-image) — leave as-is
    if (!isImageStorageConfigured())
        return value; // no public image bucket yet → keep base64
    try {
        ...
        logger_1.logger.warn('[storage] materializeImage failed — keeping base64:', err);
        return value;
    }
}
/**
 ...
 */
async function materializeImageLoose(value, coupleId) {
    if (!value || typeof value !== 'string')
        return value;
...
async function materializeImages(values, coupleId) {
    if (!values || !values.length)
        return [];
    const out = await Promise.all(values.map((v) => materializeImage(v, coupleId).then((r) => r ?? v)));
    return out.filter((v) => Boolean(v));
}
/** Best-effort delete (e.g. when a media message is deleted). Never throws. */
async function deleteObjectByUrlOrKey(urlOrKey) {
    try {
        if (!isStorageConfigured() || !urlOrKey)
            return;
        ...
        if (key.startsWith('http')) {
            ...
            key = key.replace(new RegExp(`^${env_1.env.S3_BUCKET}/`), '');
        }
        if (!key)
            return;
        await getClient().send(new client_s3_1.DeleteObjectCommand({ Bucket: bucketForKey(key), Key: key }));
    }
    catch (err) {
        logger_1.logger.warn('[storage] deleteObject failed (ignored):', err);
    }
}
